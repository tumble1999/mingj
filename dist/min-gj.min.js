...
MinGJ.bin.echo=function(n){n.shift(),console.log(n.join(" "))},MinGJ.bin.dir=function(n){var t=n.length>1?n[n.length-1]:".";t=MGJgetAbsolutePath(t),MGJPathExist(t)||console.log(t,"does not exist");var i=MGJGetObj(t);console.log(Object.keys(i).join(" "))},MinGJ.bin.cd=function(n){var t=n.length>1?n[n.length-1]:".";t=MGJgetAbsolutePath(t),MinGJ.etc.cd=t},MinGJ.bin.help=function(n){MinGJ.etc.path.forEach(n=>MinGJ.bin.dir(["dir",n]))},MinGJ.bin.mount=function(n){if(n.length<3)console.log("Usage: mount DEVICE PLACE");else{var t=MGJgetAbsolutePath(n[1]),i=MGJgetAbsolutePath(n[2]),e=MGJjoinPath(i,".."),o=i.split("/").pop();MGJGetObj(e)[o]=MGJGetObj(t)}},MinGJ.bin.debug=function(){console.info(MinGJ)},MinGJ.bin.exit=function(){console.log("This command has been disabled.")},MinGJ.bin.wget=function(n){var t=n[1],i=MGJGetObj(MGJgetAbsolutePath(".")),e=t.split("/");i[e[e.length-1]]=WEBAPIhttpGet(t)},MinGJ.bin.uname=function(n){console.log(MGJuname(n))};
/*!
 * Platform.js
