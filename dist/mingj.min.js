...
function MGJgetPrompt(){return`${MinGJ.env.username}@${location.hostname}:${MinGJ.env.cd}$ `}function MGJWhereis(n){var e=[];for(const t of MinGJ.env.path){MinGJ.getObj(t)[n]&&e.push(`${t}/${n}`)}return e}function MGJLogin(n){MinGJ.env.username=n,"object"!=typeof MinGJ.home[n]&&(MinGJ.home[n]={}),console.log(`\n${location.hostname} login: ${n}`)}fs.bin.bash=function(n,e,t){function i(){for(var n=0;t.env[n];)delete t.env[n],n++}if(n<2)MinGJ.print("JS Bash alpha 1.0");else{Object.assign(t.env,e),e.shift(),t.print(MGJgetPrompt(),e.join(" "));for(;e.length>1;){var o=e[0];if("-"!=o[0]){var s=MGJWhereis(e[0]);s.length>0?(e.shift(),t.exec(s[0],e)):t.print(`${t.env[0]}: ${e[0]}: command not found`)}else switch(o=e.shift()){case"-c":i(),Object.assign(t.env,e)}}i()}};
...
fs.bin.su=function(e,n,o){if(e<2)o.env.username="root";else{var r=n[e-1];"-s"==r&&(r="root"),o.env.username=r}n.includes("-s")&&("root"==o.env.username?(o.mkdir("/root"),o.env.cd="/root"):(o.mkdir("/home/"+o.env.username),o.env.cd="/home/"+o.env.username))};
fs.bin.uname=function(n,e,a){var o=[];function r(n,a,r=""){var i=e.includes("-a")||e.includes("--all");(e.includes("--"+n)||e.includes("-"+a)||i&&""!==r)&&o.push(r||"unknown")}r("kernel-name","v","MinGJ"),r("nodename","n",location.hostname),r("kernel-release","r"),r("kernel-version","v"),r("machine","m",platform.os),r("processor","p"),r("hardware-platform","i"),r("operating-system","o",platform.name),a.print(o.join(" "))};
fs.bin.wget=function(t,e,n){var a=e[1];n.write(n.basename(a),WEBAPIhttpGet(a))};
